        ...
        <div className='border-t border-richblack-800'>
            {courses?.length == 0? (
                ...
                courses?.map((course) => (
                    <div key={course._id}>
                        <div
                        className='hidden lg:flex justify-between items-start px-6 py-7 text-sm text-richblack-200'
                        >
                            <div className='flex flex-1 gap-x-4'>
                                ...
                                <div className='flex flex-col gap-y-3'>
                                    ...
                                    {course.status === COURSE_STATUS.DRAFT ? (
                                        ...
                                        <div className='flex w-fit px-2 py-[2px] bg-richblack-700 rounded-full justify-center items-center text-[11px] gap-2 text-yellow-50'>
                        ...
                        <div className='lg:hidden flex gap-x-3 items-start px-6 py-7 text-sm text-richblack-200'>
                            <img src={course.thumbnail} alt={`Thumbnail-${course.courseNamee}`} className='rounded-lg object-cover h-[168px] w-[220px]' />
                            <div className='flex flex-col gap-y-1'>
                                <p className='text-xl text-richblack-25'>{course?.courseName}</p>
                                <p>{course?.courseDescription.slice(0, 40)}...</p>
                                <p>Duration: 2h 30min</p>
                                <p className='text-xl text-richblack-25'>Rs. {course?.price}</p>
                                ...
                                <div className='flex gap-x-3'>
                                    <button
                                        ...
                                        onClick={() => navigate(`/dashboard/edit-course/${course._id}`)}
                                        title='Edit'
                                        className='px-2 text-xl bg-caribbeangreen-600 rounded-lg p-1'
                                    >
                                        Edit
                                    </button>
                                    ...
                                    <button
                                        ...
                                        title='Delete'
                                        className='text-xl px-2 bg-[#ff0000] rounded-lg p-1'
                                    >
                                        Delete
                                    </button>
                                </div>
